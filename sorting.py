...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    ysi = 0
    nlist = []
    while xsi < len(xs) and ysi < len(ys):
        k = cmp(xs[xsi], ys[ysi])
        if k == -1:
            nlist += [xs[xsi]]
            xsi += 1
...
def merge_sorted(xs, cmp=cmp_standard):
    ...
    '''
    xs1 = copy.deepcopy(xs)
    if len(xs1) < 2:
        return xs1
    else:
        midpoint = len(xs1) // 2
        le = merge_sorted(xs1[:midpoint], cmp)
        r = merge_sorted(xs1[midpoint:], cmp)
        return _merged(le, r, cmp)
    ...
    if len(xs1) < 2:
        ...
        for i in xs1:
            ...
            if k == -1:
                ...
                eqp += [i]
        return quick_sorted(lessp, cmp) + eqp + quick_sorted(greatp, cmp)
